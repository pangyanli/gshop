<template>
  <div  class="ratingselect">
    ...
    <div  class="switch"  :class="{on: onlyShowContent}" @click="toggleShowContent">
...
<script>
  export default{
    props:{
      desc: Object,
      onlyShowContent: Boolean,  // 仅显示文本
      ratings: Array,  // 0是推荐的，1是吐槽的
      selectType: Number  // 三种：0表示positive；1 表示nagetive；2表示全部all
    },
    computed: {
      // 计算推荐的评价数量
      positiveLength(){
    ...
    methods: {
      ...
      toggleShowContent(){
        if (!event._constructed) {  //解决兼容的
          return
        }
        // 触发事件
        this.$emit('toggleShowContent')
      }
    }
  }
</script>
